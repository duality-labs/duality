...
dasel put bool -f ${HOME}/.duality/config/app.toml -s ".api.enabled-unsafe-cors" "$([[ $IS_MAINNET ]] && echo "false" || echo "true")"
dasel put string -f ${HOME}/.duality/config/config.toml -s ".rpc.cors_allowed_origins" "$([[ $IS_MAINNET ]] && echo "app.duality.xyz" || echo "*")"
