...
createAndFundUser() {
    ...
    dualityd --home /testnet/dualityd tx bank send $(dualityd --home /testnet/dualityd keys show fred --output json | jq -r .address) $(dualityd --home /testnet/dualityd keys show $person --output json | jq -r .address) $tokens -y --broadcast-mode block >/dev/null
