...
dualityd --home /testnet/dualityd keys add $person <<< $'asdfasdf\nn'
dualityd --home /testnet/dualityd tx bank send $(dualityd --home /testnet/dualityd keys show fred --output json | jq -r .address) $(dualityd --home /testnet/dualityd keys show $person --output json | jq -r .address) 1000tokenA,1000tokenB -y --broadcast-mode block --output json

echo "$test_name: start test"
# deposit tokens
# wait for tx to be processed and return an exit code
tx_result=$(dualityd --home /testnet/dualityd tx dex deposit $(dualityd --home /testnet/dualityd keys show $person --output json | jq -r .address) tokenA tokenB 0.0000000000000001 0.0000000000000001 1 0 --from "$person" --yes --output json --broadcast-mode block)
