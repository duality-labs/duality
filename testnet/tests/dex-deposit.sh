...
# (token amounts here are measured in utoken denom)
tx_result=$(dualityd tx dex deposit $(dualityd keys show $person --output json | jq -r .address) tokenA tokenB 100 100 1 0 --from "$person" --yes --output json --broadcast-mode block)
