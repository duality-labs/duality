...
class QueryStub(betterproto.ServiceStub):
    ...
    async def subspaces(self) -> "QuerySubspacesResponse":
        ...
        return await self._unary_unary(
            "/cosmos.params.v1beta1.Query/Subspaces", request, QuerySubspacesResponse
        )
...
class QueryBase(ServiceBase):
    ...
    async def __rpc_subspaces(self, stream: grpclib.server.Stream) -> None:
        request = await stream.recv_message()

        request_kwargs = {}

        response = await self.subspaces(**request_kwargs)
        await stream.send_message(response)
