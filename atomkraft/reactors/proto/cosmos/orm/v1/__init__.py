...
class PrimaryKeyDescriptor(betterproto.Message):
    ...
    # encoding that enables sorted iteration.   - google.protobuf.Timestamp and
    # google.protobuf.Duration are encoded   as 12 bytes using an encoding that
    # enables sorted iteration.   - enum fields are encoded using varint encoding
    # and do not support sorted   iteration.   - bool fields are encoded as a
    # single byte 0 or 1. All other fields types are unsupported in keys
    # including repeated and oneof fields. Primary keys are prefixed by the
...
class SecondaryIndexDescriptor(betterproto.Message):
    ...
    # the the field segments, non-unique index keys are suffixed with any
    # additional primary key fields not present in the index fields so that the
