...
type IBCMiddleware struct {
	...
	transcoder SwapForwardMemoTranscoder
}

func NewIBCMiddleware(app porttypes.IBCModule, transcoder SwapForwardMemoTranscoder) IBCMiddleware {
...
func (im IBCMiddleware) OnRecvPacket(
	...
	case TypeGeneralMessage:
		// all functionality requires token(s) for now
		// if insufficient tokens are sent there should be a failure
		return channeltypes.NewErrorAcknowledgement(fmt.Errorf("GMP message should come with tokens for swap"))
	case TypeGeneralMessageWithToken:
		// we throw out the rest of the msg.Payload fields here, for better or worse
		var memoBz []byte
		memoBz, err = im.transcoder.Process(msg.Payload)
		if err != nil {
			return channeltypes.NewErrorAcknowledgement(fmt.Errorf("Failed to transcode ABI-encoded memo to JSON: %w", err))
		}
