...
func (s *MsgServerTestSuite) SetupTest() {
	...
	types.RegisterQueryServer(queryHelper, app.MevdummyKeeper)
	...
	s.app = app
	s.msgServer = keeper.NewMsgServerImpl(app.MevdummyKeeper)
	s.ctx = ctx
...
func (s *MsgServerTestSuite) assertMEVDummyBalancesEpsilon(
	...
	allowableError := sdk.NewInt(2)
	aActual := s.app.BankKeeper.GetBalance(s.ctx, s.app.AccountKeeper.GetModuleAddress("mevdummy"), "TokenA").Amount
	...
	bActual := s.app.BankKeeper.GetBalance(s.ctx, s.app.AccountKeeper.GetModuleAddress("mevdummy"), "TokenB").Amount
	bBalanceDelta := bBalance.Sub(bActual)
...
func (s *MsgServerTestSuite) assertMEVDummyBalances(a int64, b int64) {
	s.assertAccountBalances(s.app.AccountKeeper.GetModuleAddress("mevdummy"), a, b)
}

func (s *MsgServerTestSuite) assertMEVDummyBalancesInt(a sdk.Int, b sdk.Int) {
	s.assertAccountBalancesInt(s.app.AccountKeeper.GetModuleAddress("mevdummy"), a, b)
}
