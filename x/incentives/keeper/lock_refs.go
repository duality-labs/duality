...
func getStakeRefKeys(stake *types.Stake) ([][]byte, error) {
	...
	refKeys = append(refKeys, types.KeyPrefixStakeIndexAccount, owner)
	...
	for _, coin := range stake.Coins {
		...
		refKeys = append(refKeys, types.KeyPrefixStakeIndexDenom, denomBz)
		refKeys = append(refKeys, types.KeyPrefixStakeIndexPairTick, pairIdBz, tickBz)
		refKeys = append(refKeys, types.KeyPrefixStakeIndexAccountDenom, owner, denomBz)
		refKeys = append(refKeys, types.KeyPrefixStakeIndexPairTimestamp, pairIdBz, types.GetTimeKey(stake.StartTime))
	}
