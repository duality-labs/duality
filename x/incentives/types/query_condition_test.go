...
func TestQueryCondition(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "Matching poolMetadata and tick range",
			queryCond: QueryCondition{PairID: pairID, StartTick: 10, EndTick: 20},
			poolMetadata: dextypes.NewPoolMetadata(
				1,
				&dextypes.PairID{Token0: "coin1", Token1: "coin2"},
				15,
				5,
			),
			testResult: true,
		...
		{
			name:      "Non-matching poolMetadata",
			queryCond: QueryCondition{PairID: pairID, StartTick: 10, EndTick: 20},
			poolMetadata: dextypes.NewPoolMetadata(
				1,
				&dextypes.PairID{Token0: "coin1", Token1: "coin3"},
				15,
				5,
			),
			testResult: false,
