...
func (k Keeper) GetTotalReservesAtTick(goCtx context.Context, pairId string, tick_index_ int64, swap0to1 bool) (sdk.Dec, sdk.Dec, error) {
	...
	// retrivies tick from tickObjecting
	tick, tickFound := k.GetTickObject(ctx, pairId, tick_index_)
...
func (k Keeper) DepositHelper(goCtx context.Context, pairId string, pair types.PairObject, tickIndex int64, amount0 sdk.Dec, amount1 sdk.Dec, fee int64, feeIndex uint64) (sdk.Dec, sdk.Dec, sdk.Dec, sdk.Dec, sdk.Dec, error) {
	...
	lowerTick, lowerTickFound := k.GetTickObject(ctx, pairId, tickIndex-int64(fee))
	upperTick, upperTickFound := k.GetTickObject(ctx, pairId, tickIndex+int64(fee))
...
func (k Keeper) WithdrawCore(goCtx context.Context, msg *types.MsgWithdrawl, token0 string, token1 string, callerAddr sdk.AccAddress, receiverAddr sdk.AccAddress) error {
	...
	for i, _ := range msg.SharesToRemove {
		...
		upperTick, upperTickFound := k.GetTickObject(ctx, pairId, msg.TickIndexes[i]+int64(fee))
		lowerTick, lowerTickFound := k.GetTickObject(ctx, pairId, msg.TickIndexes[i]-int64(fee))
		...
		if isTickEmpty && upperTick.LimitOrderPool0To1.CurrentLimitOrderKey == 0 && upperTick.LimitOrderPool1To0.CurrentLimitOrderKey == 0 {
			...
			Pool0to1, Pool0to1Found := k.GetLimitOrderPoolTotalSharesObject(ctx, pairId, upperTick.TickIndex, token0, 0)
			Pool1to0, Pool1to0Found := k.GetLimitOrderPoolTotalSharesObject(ctx, pairId, upperTick.TickIndex, token1, 0)
		...
		if isTickEmpty && (msg.TickIndexes[i]+int64(fee) == pair.TokenPair.CurrentTick1To0) {
			...
			for tickFound != true && pair.TotalTickCount < int64(c) {
				...
				_, tickFound = k.GetTickObject(ctx, pairId, (msg.TickIndexes[i] + fee + int64(c)))
...
func (k Keeper) Swap0to1(goCtx context.Context, msg *types.MsgSwap, token0 string, token1 string, callerAddr sdk.AccAddress) (sdk.Dec, error) {
	...
	for !amount_left.Equal(sdk.ZeroDec()) && (count < int(pair.TotalTickCount)) {
		...
		Current1Data, Current1Found := k.GetTickObject(ctx, pairId, pair.TokenPair.CurrentTick0To1)
		...
		for i < feeSize && !amount_left.Equal(sdk.ZeroDec()) {
			...
			Current0Data, Current0Found := k.GetTickObject(ctx, pairId, pair.TokenPair.CurrentTick0To1+2*fee)
			...
			//Make updates to tickObject containing reserve0/1 data to the KVStore
			k.SetTickObject(ctx, pairId, Current0Data)

		}
		k.SetTickObject(ctx, pairId, Current1Data)
...
func (k Keeper) Swap1to0(goCtx context.Context, msg *types.MsgSwap, token0 string, token1 string, callerAddr sdk.AccAddress) (sdk.Dec, error) {
	...
	for !amount_left.Equal(sdk.ZeroDec()) && (count < int(pair.TotalTickCount)) {
		...
		Current0Data, Current0Found := k.GetTickObject(ctx, pairId, pair.TokenPair.CurrentTick1To0)
		//Current0Datam := Current0Data.TickData.Reserve1[i]
		...
		for i < feeSize && !amount_left.Equal(sdk.ZeroDec()) {
			...
			Current1Data, Current1Found := k.GetTickObject(ctx, pairId, pair.TokenPair.CurrentTick1To0-2*fee)
...
func (k Keeper) SwapLimitOrder0to1(goCtx context.Context, pairId string, tokenIn string, amount_out sdk.Dec, amount_left sdk.Dec, CurrentTick1to0 int64) (sdk.Dec, sdk.Dec, error) {
	...
	tick, tickFound := k.GetTickObject(ctx, pairId, CurrentTick1to0)
	...
	ReserveData, ReserveDataFound := k.GetLimitOrderPoolReserveObject(ctx, pairId, CurrentTick1to0, tokenIn, tick.LimitOrderPool1To0.CurrentLimitOrderKey)
	FillData, _ := k.GetLimitOrderPoolFillObject(ctx, pairId, CurrentTick1to0, tokenIn, tick.LimitOrderPool1To0.CurrentLimitOrderKey)
	...
	if price.Mul(ReserveData.Reserves).LT(amount_left) {
		...
		ReserveDataNextKey, ReserveDataNextKeyFound := k.GetLimitOrderPoolReserveObject(ctx, pairId, CurrentTick1to0, tokenIn, tick.LimitOrderPool1To0.CurrentLimitOrderKey)
		FillDataNextKey, FillDataNextKeyFound := k.GetLimitOrderPoolFillObject(ctx, pairId, CurrentTick1to0, tokenIn, tick.LimitOrderPool1To0.CurrentLimitOrderKey)
		...
		k.SetLimitOrderPoolFillObject(ctx, FillDataNextKey)
		k.SetLimitOrderPoolReserveObject(ctx, ReserveDataNextKey)
	...
	k.SetLimitOrderPoolReserveObject(ctx, ReserveData)
	...
	k.SetTickObject(ctx, pairId, tick)
...
func (k Keeper) SwapLimitOrder1to0(goCtx context.Context, pairId string, tokenIn string, amount_out sdk.Dec, amount_left sdk.Dec, CurrentTick0to1 int64) (sdk.Dec, sdk.Dec, error) {
	...
	tick, tickFound := k.GetTickObject(ctx, pairId, CurrentTick0to1)
	...
	ReserveData, ReserveDataFound := k.GetLimitOrderPoolReserveObject(ctx, pairId, CurrentTick0to1, tokenIn, tick.LimitOrderPool0To1.CurrentLimitOrderKey)
	FillData, _ := k.GetLimitOrderPoolFillObject(ctx, pairId, CurrentTick0to1, tokenIn, tick.LimitOrderPool0To1.CurrentLimitOrderKey)
	...
	if price.Mul(ReserveData.Reserves).LT(amount_left) {
		...
		// checks the next currentLimitOrderKey
		ReserveDataNextKey, ReserveDataNextKeyFound := k.GetLimitOrderPoolReserveObject(ctx, pairId, CurrentTick0to1, tokenIn, tick.LimitOrderPool0To1.CurrentLimitOrderKey)
		FillDataNextKey, FillMapNextKeyFound := k.GetLimitOrderPoolFillObject(ctx, pairId, CurrentTick0to1, tokenIn, tick.LimitOrderPool0To1.CurrentLimitOrderKey)
...
func (k Keeper) PlaceLimitOrderHelper(goCtx context.Context, pairId string, tickIndex int64) types.TickObject {
	ctx := sdk.UnwrapSDKContext(goCtx)

	// returns tick object for tick specified by tickIndex
	tick, tickFound := k.GetTickObject(ctx, pairId, tickIndex)
...
func (k Keeper) PlaceLimitOrderMappingHelper(goCtx context.Context, pairId string, tickIndex int64, tokenIn string, currentLimitOrderKey uint64, receiver string) (types.LimitOrderPoolReserveObject, types.LimitOrderPoolUserShareObject, types.LimitOrderPoolTotalSharesObject) {

	ctx := sdk.UnwrapSDKContext(goCtx)

	// Retrieves ReserveData Object from KVStore
	ReserveData, ReserveDataFound := k.GetLimitOrderPoolReserveObject(ctx, pairId, tickIndex, tokenIn, currentLimitOrderKey)
	// Retrieves UserShareMap object from KVStore
	UserShareData, UserShareDataFound := k.GetLimitOrderPoolUserShareObject(ctx, pairId, tickIndex, tokenIn, currentLimitOrderKey, receiver)
	// Retrives TotalSharesMap object from KVStore
	TotalSharesData, TotalSharesDataFound := k.GetLimitOrderPoolTotalSharesObject(ctx, pairId, tickIndex, tokenIn, currentLimitOrderKey)
...
func (k Keeper) PlaceLimitOrderCore(goCtx context.Context, msg *types.MsgPlaceLimitOrder, token0 string, token1 string, callerAddr sdk.AccAddress) error {
	...
	/// tick is an object of the tickObjectping for the specified tickIndex and mapping
	...
	FillData, FillDataFound := k.GetLimitOrderPoolFillObject(ctx, pairId, msg.TickIndex, msg.TokenIn, currentLimitOrderKey)
...
func (k Keeper) CancelLimitOrderCore(goCtx context.Context, msg *types.MsgCancelLimitOrder, token0 string, token1 string, callerAddr sdk.AccAddress, receiverAddr sdk.AccAddress) error {
	...
	pairId := k.CreatePairId(token0, token1)
	// Retrives TickObject object from KVStore
	_, tickFound := k.GetTickObject(ctx, pairId, msg.TickIndex)
	...
	UserSharesData, UserSharesDataFound := k.GetLimitOrderPoolUserShareObject(ctx, pairId, msg.TickIndex, msg.KeyToken, msg.Key, msg.Creator)
	// Retrieves LimitOrderReserevMap object from KVStore for the specified key and keyToken
	ReserveData, ReserveDataFound := k.GetLimitOrderPoolReserveObject(ctx, pairId, msg.TickIndex, msg.KeyToken, msg.Key)
	// Retrieves LimitOrderTotalSharesMap object from KVStore for the specified key and keyToken
	TotalSharesData, TotalShareDataFound := k.GetLimitOrderPoolTotalSharesObject(ctx, pairId, msg.TickIndex, msg.KeyToken, msg.Key)
	...
	k.SetLimitOrderPoolUserShareObject(ctx, UserSharesData)
	k.SetLimitOrderPoolReserveObject(ctx, ReserveData)
	k.SetLimitOrderPoolTotalSharesObject(ctx, TotalSharesData)
...
func (k Keeper) WithdrawFilledLimitOrderCore(goCtx context.Context, msg *types.MsgWithdrawFilledLimitOrder, token0 string, token1 string, callerAddr sdk.AccAddress, receiverAddr sdk.AccAddress) error {
	...
	FillData, FillDataFound := k.GetLimitOrderPoolFillObject(ctx, pairId, msg.TickIndex, msg.KeyToken, msg.Key)
