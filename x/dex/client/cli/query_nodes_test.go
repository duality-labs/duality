...
func networkWithNodesObjects(t *testing.T, n int) (*network.Network, []types.Nodes) {
	...
	return network.New(t, cfg), state.NodesList
}

func TestShowNodes(t *testing.T) {
	net, objs := networkWithNodesObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetNodesResponse
...
func TestListNodes(t *testing.T) {
	net, objs := networkWithNodesObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListNodes(), args)
			require.NoError(t, err)
			var resp types.QueryAllNodesResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
